...
C 11/5/99 - Reword 99000 FORMAT in SEARCH & 99003 FORMATin REACT.
...
C
      SAVE
      PARAMETER (MAXNGC=400,MAXNC=200,NCOL=8,MAXMAT=40)
      PARAMETER (MAXR=24,MAXEL=15,MAXNG=300)
     ...
     &                Nfz,Npp,Nsub,Nsup,Page1,Rkt
      ...
      WRITE (*,20)
 20   FORMAT (//' ENTER PREFIX ONLY FOR INPUT FILE, SUFFIX SHOULD BE', 
     &' .inp'/'   THE OUTPUT FILES FOR LISTING AND PLOTTING WILL HAVE',
     & /' THE SAME PREFIX WITH SUFFIXES .out AND .plt RESPECTIVELY'//)
      READ (*,30) prefix
 30   FORMAT (a)
      ln = INDEX(prefix,' ') - 1
...
C***********************************************************************
      SAVE
      PARAMETER (MAXNGC=400,MAXNC=200,NCOL=8,MAXMAT=40,MAXTR=30)
      PARAMETER (MAXR=24,MAXEL=15,MAXNG=300)
      PARAMETER (MAXMIX=52,MAXT=51,MAXPV=26)
      DOUBLE PRECISION Deln,En,Enln,Enn,Ennl,Enlsav,Ensave,Sln,Sumn
      COMMON /COMP  / Deln(MAXNGC),En(MAXNGC,NCOL),Enln(MAXNGC),Enn,
...
C   CALCULATE CONTROL FACTOR,AMBDA
          ...
          sum = sum*5.d0
          DO 940 j = 1,Ng
            IF ( Deln(j).GT.0.d0 ) THEN
              IF ( (Enln(j)-Ennl+Size).LE.0.d0 ) THEN
...
C***********************************************************************
      SAVE
      PARAMETER (MAXNGC=400,MAXMAT=40)
      PARAMETER (MAXEL=15)
      COMMON /MISCI / Imat,Iq1,Isv,Jliq,Jsol,Lsave,Msing
      DOUBLE PRECISION A,Atwt,Avgdr,Boltz,B0,Eqrat,G,Hsub0,Oxfl,Pi,Pp,
     &                 R,Rr,Size,S0,Tln,Tm,Trace,Tt,Viscns,Vv,X
